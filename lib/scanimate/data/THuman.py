...
def load_trimesh(ply_files, param_files):
    ...
    key_name = ply_files[0].split('/')[-2].split('_')[0]
    ...
    for f, smpl_file in tqdm(zip(ply_files, param_files)):
        key_name = f.split('/')[-2].split('_')[0]
        tmp_mesh = trimesh.load(f, process=False, maintain_order=True)

        smpl_data = np.load(smpl_file)
        tmp_mesh.vertices -= smpl_data['scan_transl']
        tmp_mesh.vertices /= smpl_data['scan_scale']
...
class THumanDataset_scan(Dataset):  # Single subject under data directory!
    def __init__(self, opt, phase='train', smpl=None, smpl_processing=True,
        ...
        self.smpl_root = self.root.replace('scans', 'fits-smpl')
        self.subjects = ['scan']
        ...
        for k, v in self.pkl_data.items():
            ...
            for f_name, i in v:
                try:
                    pkl_data_list.append(os.path.join(self.smpl_root, f_name))
                    ply_data_list.append(glob(os.path.join(self.root, '*' + self.ext))[0])
                    data_ids.append(i)
                except:
                    logging.info(os.path.join(self.root, f_name[:-4] + self.ext))
                    invalid_pair += 1
    ...
    def smpl_process(self):
        ...
        for pkl_file in tqdm(self.pkl_data_list):
            ...
            dic_key = ply_file.split('/')[-2].split('_')[0]

            smpl_data = np.load(pkl_file)
            betas = torch.zeros(10).float()
            body_pose = torch.Tensor(smpl_data['pose']).view(72,)
            ...
            #     save_obj_mesh('/media/liaotingting/usb3/mesh.obj', mesh.vertices, mesh.faces)
            # save_obj_mesh('/media/liaotingting/usb3/smpl.obj', output.vertices[0].cpu().numpy())
            # print('Done')
            # exit()
    ...
    def get_subjects_minimal(self):
        file = os.path.join(self.smpl_root, 'smpl_shaped.obj')
    ...
    def collect_data(self, frame_ratio=None):
        ...
        for sub in self.subjects:
            ...
            data[sub] = sorted([f for f in os.listdir(self.smpl_root) if '.npz' in f])
            if self.is_train:
                if frame_ratio:
                    pick_flag = []
                    if frame_ratio == 1.0:
    ...
    def compute_valid_tri(self, lbs_net_p, smpl, lat_vecs_lbs_p, smpl_vitruvian):
        ...
        with torch.no_grad():
            for index in range(len(self.pkl_data_list)):
                ...
                ply_file = pkl_file[:-4] + self.ext
                ply_key = ply_file.split('/')[-2].split('_')[0]
                flag_tri = self.g_flag_tri[ply_key].to(self.device)

                smpl_data = np.load(pkl_file)
                betas = torch.Tensor(smpl_data['betas'])[None].to(self.device)
