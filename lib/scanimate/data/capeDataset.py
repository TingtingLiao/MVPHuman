...
from ..utils.ext_trimesh import sample_surface_wnormal, sample_surface_wnormalcolor, slice_mesh_plane_with_mesh_color
...
def load_trimesh(ply_files, ground_level=None):
    ...
    for f in tqdm(ply_files):
        ...
        if ground_level:
            ...
            if tmp_mesh.vertices[:, 1].max() - tmp_mesh.vertices[:,
                                               1].min() > 10:  # Not in meter, very likely in milimeter
...
class CapeDataset_scan(Dataset):  # Single subject under data directory!
    ...
    def get_subjects(self):
        if os.path.isfile(os.path.join(self.root, '%s_list.txt' % (self.phase))):
            ...
            subjects = sorted(
                [f for f in os.listdir(self.root) if os.path.isdir(os.path.join(self.root, f)) and not 'textured' in f])
            return subjects
    ...
    def collect_data(self, frame_ratio=None):
        ...
        for sub in self.subjects:
            if self.is_train:
                data[sub] = sorted([f for f in os.listdir(os.path.join(self.root, sub)) if
                                    '.pkl' in f and os.path.isfile(os.path.join(self.root, sub, f[:-4] + '.ply'))])
                if len(data[sub]) == 0:
                    data[sub] = sorted([f for f in os.listdir(os.path.join(self.root, sub)) if
                                        '.npz' in f and os.path.isfile(os.path.join(self.root, sub, f[:-4] + '.ply'))])
